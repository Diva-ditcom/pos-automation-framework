    ...
    if basket_controls:
        ...
        for idx, item in enumerate(items, 1):
            ...
            if "promotion" in name.lower() or "bonus" in name.lower() or "ff" in name.lower():
                promotion_found = True
                print(f"[SUCCESS] Promotion found in basket item: {name}")
        ...
        if promotion_found:
            print("[SUCCESS] Basket contains promotion items.")
            time.sleep(3)
        ...
        print("[ERROR] Could not find basket/list control on left side.")
        time.sleep(2)
        click_button_by_text(win, "OK")
        return True
...
def main():
    ...
    if not check_basket(win):
        print("[ERROR] Failed to check basket")
        return
